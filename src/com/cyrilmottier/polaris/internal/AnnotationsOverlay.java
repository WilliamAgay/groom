...
//        void dismissCallout(int position);
        void dismissCallout(int position, boolean isAnimated);

//        void showCallout(int position);
        void showCallout(int position, boolean isAnimated);
    }
	...
	private final boolean isAnimatedDefault = true;
    ...
    protected boolean onTap(int index) {
        if (index >= 0 && index < size()) {
            setSelectedAnnotation(index, isAnimatedDefault );
            return true;
        }
    ...
    public void setSelectedAnnotation(int position, boolean isAnimated) {
        ...
        if (position != mSelectedAnnotation) {
            ...
            if (mSelectedAnnotation != INVALID_POSITION) {
                mCallback.dismissCallout(mSelectedAnnotation, isAnimated);
            }
            mSelectedAnnotation = position;
            if (position != INVALID_POSITION) {
                mCallback.showCallout(position, isAnimated);
            }
        }
    }
