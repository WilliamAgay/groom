...
public class PolarisMapView extends MapView {
    ...
    public void setAnnotations(List<Annotation> annotations, Drawable annotationMarker) {
        ...
        if (mAnnotationsOverlay != null) {
            mAnnotationsOverlay.setSelectedAnnotation(INVALID_POSITION,false);
        }
     ...
     */
    public void setSelectedAnnotation(int position, boolean isAnimated) {
        if (mAnnotationsOverlay != null) {
            mAnnotationsOverlay.setSelectedAnnotation(position, isAnimated);
        }
    }
    ...
    private final MystiqueCallback mMystiqueCallback = new MystiqueCallback() {
        public void dismissCallout(int position, boolean isAnimated) {
            ...
            if (mapCalloutView != null && mapCalloutView.getVisibility() == View.VISIBLE) {
                mapCalloutView.dismiss(isAnimated);
        ...
        public void showCallout(int position, boolean isAnimated) {
            final Annotation annotation = mAnnotationsOverlay.getAnnotation(position);
            if (annotation == null) {
                return;
            }

            dismissCallout(position,isAnimated);
            ...
            if (mapCalloutView.hasDisplayableContent()) {
                mapCalloutView.show(PolarisMapView.this, annotation.getPoint(), isAnimated);
            }
    ...
    private final MagnetoCallback mMagnetoCallback = new MagnetoCallback() {
        public void onSimpleTap(MotionEvent e) {
            setSelectedAnnotation(INVALID_POSITION, true);
        }
