...
public class GroomApplication extends Application {
	...
	public List<String> theme_pleinAirTTS = Arrays
			.asList("respirer", "air", "plein air", "espace", "détente",
					"environnement", "vert", "espace");
	public List<String> theme_restaurationTTS = Arrays.asList("restaurant",
			"restauration", "manger", "déguster", "fast food", "appétit");
	public List<String> theme_sportTTS = Arrays.asList("sport", "santé",
			"motivation", "bouger", "courrir", "se dégourdir", "endurer");
	public List<String> theme_cultureTTS = Arrays.asList("culture", "cultiver",
			"penser", "musique", "théàtre", "cinéma");
	...
	public void readThemeDataInPref() {
		...
		if (prefs.contains(PREF_CULTURE))
			prefCultureSelected = prefs.getBoolean(PREF_CULTURE, false);
		if (prefs.contains(PREF_PLEINAIR))
			prefPleinAirSelected = prefs.getBoolean(PREF_PLEINAIR, false);
		if (prefs.contains(PREF_SPORT))
			prefSportSelected = prefs.getBoolean(PREF_SPORT, false);
		if (prefs.contains(PREF_RESTO))
			prefRestoSelected = prefs.getBoolean(PREF_RESTO, false);
	...
	public void fillThemes() {
		...
		if (prefCultureSelected)
			themes.add(DataprovenceManager.THEME_CULTURE);
		if (prefPleinAirSelected)
			themes.add(DataprovenceManager.THEME_PLEINAIR);
		if (prefSportSelected)
			themes.add(DataprovenceManager.THEME_SPORT);
		if (prefRestoSelected)
			themes.add(DataprovenceManager.THEME_RESTAURATION);
